...
`effectiveJava.chapter16/Price`는 할인 결과를 새 객체로 반환한다. 같은 패키지의 `Product`는 String 목록을 `List.copyOf`로 보관한다. `ImmutableExampleTest`의 7개 테스트로 경계값과 입력·출력 보호, 가변 record·얕은 복사·원소 복사를 비교한다.

```sh
./gradlew test --tests 'dev.study.javalab.effectiveJava.chapter16.*'
```

[학습 노트와 그림](../01-Java-기본기와-실행-원리/이펙티브-Java/16-변경%20가능한%20상태%20줄이기/학습%20노트.md) · [실습 기록](../01-Java-기본기와-실행-원리/이펙티브-Java/16-변경%20가능한%20상태%20줄이기/실습%20기록.md)
