...
![호출자에서 프록시와 서비스를 거쳐 트랜잭션을 종료하는 흐름. 정상 처리 시 주문 1건·재고 7개, 예외 전달 시 주문 0건·재고 10개, 예외를 잡고 false 반환 시 주문 1건·재고 10개가 된다.](images/transaction-flow.png)

그림 1. 두 `Spring 프록시` 상자는 **같은 프록시의 호출 전 처리와 호출 후 처리**를 시간 순서대로 표현한다. 프록시는 트랜잭션 매니저에 시작·종료를 요청하고, 종료 처리와 자원 정리 후 호출자에게 결과 또는 예외를 전달한다. 아래 세 카드는 각각 초기 주문 0건·재고 10개로 시작하는 독립된 실습이다. 세 번째 케이스는 서비스 안에서 직접 발생한 예외를 잡았으며 롤백 전용 상태가 아닌 경우다.
